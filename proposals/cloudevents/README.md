...
- **event-id** - Requiered, unique ID of the event, used for invocation logging and tracking 
- **event-type** - Requiered, identify the unique event class/kind/ver 
...
- **content-type** - Requiered if contain a body, the body encoding scheme (e.g. application/json)
...
- the event message content is very similar in both proposals, the key difference is that open-events suggests two layers of serialization, the HTTP body is of type JSON, and the data can be of different type (e.g. XML, protobuf, bin, ..), while this proposal suggest to put all metadata in headers (e.g. HTTP, AMQP, etc.), The downside of open-events approach are::
  ...
  - it adds complexity (e.g. an existing event source used to just send a body need to re-format the message and embed the body as a nested object under the event, cant use postman or other tools as-is)
